...
export class CommentStore {
  ...
  _createCollabSharedMap(commentOrThread: Comment | Thread): YMap<any> {
    ...
    sharedMap.set("id", id);
    if (type === "comment") {
      sharedMap.set("author", (commentOrThread as Comment).author);
      sharedMap.set("content", (commentOrThread as Comment).content);
      sharedMap.set("deleted", (commentOrThread as Comment).deleted);
      sharedMap.set("timeStamp", (commentOrThread as Comment).timeStamp);
    } else {
      sharedMap.set("quote", (commentOrThread as Thread).quote);
      const commentsArray = new YArray();
      (commentOrThread as Thread).comments.forEach((comment: any, i: any) => {
                ...
                for (let d = 0; d < del; d++) {
                  const commentOrThread =
                    parentThread === undefined
                      ...
                      : (parentThread as Thread).comments[offset];
                  this._withLocalTransaction(() => {
